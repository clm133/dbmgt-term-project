...
CREATE OR REPLACE FUNCTION group_limit_reached(gID NUMBER)
	...
	BEGIN
		...
		IF total_membership = group_limit THEN is_reached := 1; --1 indicates group limit reached
		ELSE is_reached := 0; --indicates group limit is not reached
		END IF;
		
		RETURN(is_reached);
	END;
/
	/*Actual Constraint Regarding Group Membership */
